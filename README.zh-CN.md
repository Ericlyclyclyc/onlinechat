...
* 🚫 零额外运行时依赖 —— Netty 与 Gson 来自 Minecraft 自身；1.21.1 缺的那一个模块（`netty-codec-http`）通过 JarInJar 内置在 jar 中。
...
|------|-----------|----------|-----------|---------|-----------|----------------|
| **`master`** *（本分支）* | 1.21.1 | 21.1.233+ | `neoforge` | 21 | ModDevGradle 2.0.147 · Gradle 9.2.1 | `onlinechat-1.21.1-neoforge-<版本>.jar` |
...
* MC 1.21.1 自带的 Netty 4.1.97 **不含** `netty-codec-http`，因此本分支通过 JarInJar
  把它直接内置进发布 jar（不打包任何其他东西）。
* 1.21.1 的服务端配置位于 `config/onlinechat-server.toml`
  （1.20.1 分支则每世界一份，位于 `world/serverconfig/`）。
