...
| Denial of service | Exhaust server resources | Per-IP **and** global WebSocket connection caps, per-IP login/register limits, per-session chat-message limit, per-account bind limit (all shared across connections and keyed per IP/account, not per TCP connection). HTTP body capped at 1 MiB, WebSocket frame capped at 64 KiB, 120 s idle timeout, chat history ring-bounded. Full-archive search requires auth, is rate-limited (30/min/IP) and paginated, so it cannot be used as an amplification vector. PBKDF2 hashing and account file writes are offloaded to a bounded worker pool so they never block the Netty event loop. |
...
| Replay of chat history | Leak private conversation | History and archive search are only served to authenticated sessions. |
